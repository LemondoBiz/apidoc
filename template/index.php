...
function isLoggedIn()
{
    ...
    if (!isset($_SESSION["username"]))
        header("Location:http://apidoc.lemon.do/login.php");
}

function checkApiDocAccess()
{
    $id = $_SESSION['userid'];
    ...
    $url = explode("/", $url);
    $path = $url[3];
    $res = select_db("SELECT 1 FROM apis WHERE userid = " . $id . " AND title = N'" . $path . "'");
    if ($res) return "1";
    else return "0";
}

isLoggedIn();
if(checkApiDocAccess() == "0")header("Location:http://apidoc.lemon.do/login.php");
?>
<!DOCTYPE html>
<html>
<head>
...
<script id="template-generator" type="text/x-handlebars-template">
    ...
    {{#if generator}}
    <div class="content">
        <a href="mailto:support@lemondo.ge">support@lemondo.ge</a>
    </div>
    {{/if}}
    {{/if}}
</script>
...
<script id="template-article" type="text/x-handlebars-template">
    ...
    <div class="pull-left">
        <h1>{{#if article.title}}  {{article.title}}{{/if}}</h1>
    </div>
    {{#if template.withCompare}}
    <div class="pull-right">
        <div class="btn-group">
...
<script data-main="print.js" src="utils/print.js"></script>
